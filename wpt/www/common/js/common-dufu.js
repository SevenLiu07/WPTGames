...
PLAY.baseUrl = http_type + location.host + '/';
PLAY.staticUrl = http_type + "static.play-wpt.com" + "/";
PLAY.pokerUrl = PLAY.baseUrl + "poker";
PLAY.gameSlotUrl = PLAY.baseUrl + "game_slot.html";
PLAY.pokerUrlNoVersion = PLAY.baseUrl + "poker";
PLAY.indexUrl = PLAY.baseUrl;
PLAY.gamePokerUrl = "";
PLAY.baseAccountUrl = "";
if (location.href.indexOf('staging') !== -1) {
    PLAY.webApiUrl = http_type + "staging-napi." + getHost[1] + '.com/';
    PLAY.leaderBoardUrl = http_type + "staging-napi." + getHost[1] + ".com/";
    PLAY.shopUrl = http_type + "staging-napi." + getHost[1] + ".com/";
    PLAY.baseAccountUrl = http_type + "staging-accounts." + getHost[1] + ".com/";
    PLAY.gamePokerUrl = http_type + "staging-games." + getHost[1] + ".com/";
} else {
    PLAY.webApiUrl = http_type + "napi." + getHost[1] + '.com/';
    PLAY.leaderBoardUrl = http_type + "napi." + getHost[1] + ".com/";
    PLAY.shopUrl = http_type + "napi." + getHost[1] + ".com/";
    PLAY.baseAccountUrl = http_type + "accounts." + getHost[1] + ".com/";
    PLAY.gamePokerUrl = http_type + "games." + getHost[1] + ".com/";
}
...
//统一处理弹窗缩放
/*缩放操作*/
PLAY.scaleDialog = function () {
    if ($(window).width() <= 1445) {
        $(".widget-dialog").css("transform", "scale(0.7)");
    }
    PLAY.resizeWin();
};
/*恢复缩放操作*/
PLAY.resetScaleDialog = function () {
    if ($(window).width() <= 1445) {
        $(".widget-dialog").css("transform", "scale(1)");
    }
    PLAY.resizeWin();
};
/*resize window*/
PLAY.resizeWin = function () {
...
if (location.href.indexOf("new_vote.html") !== -1 || location.href.indexOf("vote_end.html") !== -1) {
} else {
    if (!PLAY.userInfo) {
        /*获取登录信息*/
        PLAY.ajax.Get({
