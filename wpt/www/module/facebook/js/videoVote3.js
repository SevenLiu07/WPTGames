require(["../../model/vote-model.js", "domReady!"], function (voteModel) {
            ...
            voteModel.getVideosByGroupCode(param, function (res) {
                if (res.Value) {
                    ...
                    if (res.Value.VoteGroup && res.Value.VoteGroup.IsFinished) {
                        ...
                        location.href = PLAY.baseUrl + "wpt/module/facebook/vote_end.html";
                    } else {
                        var voteVideoCookie = PLAY.cookie(vm.voteVideoCookie);
                        if (voteVideoCookie) {
                            videoVote.hasVote = true;
                            videoVote.voteBtnText = "Thanks for voting";
                            $(".vote_button").css({
                                "opacity": 0.7,
                                "cursor": "default"
                            });
        ...
        vm.clickHeader = function (param) {
            ...
            if (IsIpad || IsIOS) {
                ...
                if (param) {
                    ...
                    $("#playNowBtn").attr("href", PLAY.pokerUrl)
                } else {
                    //location.href = PLAY.indexUrl;
                    $("#playNowBtn").attr("href", PLAY.indexUrl)
                }
            }
        };
        /*判断登录*/
